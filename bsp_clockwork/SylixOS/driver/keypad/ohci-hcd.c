...
#ifdef DEBUG
#define dbg(format, arg...) _PrintFormat("HCD DEBUG: " format, ## arg)
#else
#define dbg(format, arg...) do {} while (0)
#endif /* DEBUG */
#define err(format, arg...) _PrintFormat("HCD ERROR: " format, ## arg)
#ifdef SHOW_INFO
#define info(format, arg...) _PrintFormat("HCD INFO: " format, ## arg)
#else
...
//#define flush_dcache_buffer(addr, size)     API_CacheFlush(DATA_CACHE, addr, size)
//#define invalidate_dcache_buffer(addr, size)    API_CacheInvalidate(DATA_CACHE, addr, size)
...
#define ohci_mdelay(x) API_TimeMSleep(x)

/* global ohci_t */
static ohci_t *gohci;
/* this must be aligned to a 256 byte boundary */
struct ohci_hcca *ghcca;
		      ...
		      struct devrequest *setup, char *str, int small)
{
	...
	if (!small) {
		...
		if (usb_pipecontrol(pipe)) {
		    _PrintFormat(__FILE__ ": cmd(8):");
			for (i = 0; i < 8 ; i++)
			    _PrintFormat(" %02x", ((__u8 *) setup) [i]);
			_PrintFormat("\r\n");
		}
		if (transfer_len > 0 && buffer) {
		    _PrintFormat(__FILE__ ": data(%d/%d):",
				(purb ? purb->actual_length : 0),
				transfer_len);
			len = usb_pipeout(pipe)? transfer_len:
					(purb ? purb->actual_length : 0);
			for (i = 0; i < 16 && i < len; i++)
			    _PrintFormat(" %02x", ((__u8 *) buffer) [i]);
			_PrintFormat("%s\r\n", i < len? "...": "");
		}
	}
#endif
...
void ep_print_int_eds(ohci_t *ohci, char *str)
{
	...
	for (i = 0; i < 32; i++) {
		...
		_PrintFormat(__FILE__ ": %s branch int %2d(%2x):", str, i, i);
		while (*ed_p != 0 && j--) {
			ed_t *ed = (ed_t *)m32_swap(ed_p);
			invalidate_dcache_ed(ed);
			_PrintFormat(" ed: %4x;", ed->hwINFO);
			ed_p = &ed->hwNextED;
		}
		_PrintFormat("\r\n");
	}
...
static void print_tds(void *start)
{
    td_t *tdp = start;
    int i = 0;
...
{
	dbg("OHCI controller usb-%s state\r\n", controller->slot_name);
	...
	struct usb_device *dev, int index, urb_priv_t *urb_priv)
{
	...
	if (usb_pipebulk(urb_priv->pipe) && usb_pipeout(urb_priv->pipe)) {
		for (i = 0; i < len; i++)
		    _PrintFormat("td->data[%d] %2x ", i, ((unsigned char *)td->data)[i]);
		_PrintFormat("\r\n");
	}
#endif
	if (!len)
...
#ifdef DEBUG
#define WR_RH_STAT(x)		{info("WR:status %8x\r\n", (x)); ohci_writel((x), \
						&ohci->regs->roothub.status); }
#define WR_RH_PORTSTAT(x)	{info("WR:portstatus[%d] %8x\r\n", wIndex-1, \
...
static ohci_dev_t *ohci_get_ohci_dev(ohci_t *ohci, int devnum, int intr)
{
	...
	_PrintFormat("ohci: Error out of ohci_devs for interrupt endpoints\r\n");
	return NULL;
}
		...
		void *buffer, int transfer_len, int interval)
{
	...
	if (!urb) {
	    _PrintFormat("ohci: Error out of memory allocating urb\r\n");
		return NULL;
	}
		...
		struct devrequest *setup, int interval)
{
    ...
    API_SemaphoreBPend(ohci->sync_pipe, LW_OPTION_WAIT_INFINITE);
#if 0
        for (;;) {
		...
		int elementsize, void *buffer, int interval)
{
	...
	if (!queue) {
	    _PrintFormat("ohci: Error out of memory allocating int queue\r\n");
		return NULL;
	}
	...
	for (i = 0; i < queuesize; i++) {
		...
		if (sohci_submit_job(ohci, ohci_dev, queue->urb[i], NULL)) {
		    _PrintFormat("ohci: Error submitting int queue job\r\n");
...
//	mask = OHCI_INTR_RHSC | OHCI_INTR_UE | OHCI_INTR_WDH | OHCI_INTR_SO;
	mask = OHCI_INTR_UE | OHCI_INTR_WDH | OHCI_INTR_SO | OHCI_INTR_MIE;
...
extern struct usb_device *dpad_dev;
//handle int pipe
static PVOID t_pipe (PVOID pvArg)
{
    for (;;) {
        if (!dpad_dev) {
            sleep(1);
        ...
        if (dpad_dev->irq_handle)
            dpad_dev->irq_handle(dpad_dev);
        else
            sleep(1);
    }
...
static irqreturn_t usb_irq (PVOID pvArg, ULONG ulVector)
{
    int stat = 0;
    ohci_t *pohci = (ohci_t *)pvArg;
    urb_priv_t *urb = pohci->urb;
     ...
     * hc_interrupt() gets called again as there needs to be some
     * more TD's to process still */
    if ((stat >= 0) && (stat != 0xff) && urb && (urb->finished)) {
        /* 0xff is returned for an SF-interrupt */
        API_SemaphoreBPost(pohci->sync_pipe);
    }
...
int usb_lowlevel_init(int index, enum usb_init_type init, void **controller)
{
	...
	gohci = API_VmmDmaAllocAlign(sizeof(ohci_t), 32);
	memset(gohci, 0, sizeof(ohci_t));

	ghcca = API_VmmDmaAllocAlign(sizeof(struct ohci_hcca), 256);

	memset(ghcca, 0, sizeof(struct ohci_hcca));

	gohci->hcca = ghcca;
	info("aligned ghcca %p\r\n", gohci->hcca);
	...
	gohci->sync_pipe = API_SemaphoreBCreate("sync_pipe",
                                            FALSE,
                                            LW_OPTION_OBJECT_GLOBAL | LW_OPTION_INHERIT_PRIORITY,
                                            NULL);

	LW_CLASS_THREADATTR threadattr;
	threadattr = API_ThreadAttrGetDefault();
	threadattr.THREADATTR_pvArg = (PVOID)gohci;
    API_ThreadCreate("t_pipe", t_pipe, &threadattr, LW_NULL);
    ...
    API_InterVectorConnect(105,
                           usb_irq,
                           (PVOID)gohci,
                           "usb_isr");
    API_InterVectorEnable(105);

#ifdef USE_IN_MODULE
    LW_CLASS_CPUSET set;
    LW_CPU_ZERO(&set);
    LW_CPU_SET(1, &set);
    API_InterSetTarget(105, sizeof(LW_CLASS_CPUSET), &set);
#endif
